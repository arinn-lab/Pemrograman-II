...
public class DeliveryController {
    ...
    @FXML private ComboBox<String> cmbServiceType; // Pilihan: Regular / Express
    ...
    @FXML private TableColumn<BasePackage, Double> colCost; // Kolom Harga (Hitung Otomatis)

    private DeliveryService service = new DeliveryRepository();
    ...
    private void handleAdd() {
        if (isInputValid()) {
            ...
            if (type.equals("Regular Service")) {
                newPackage = new RegularPackage(receipt, sender, dest, weight);
            } else {
                newPackage = new ExpressPackage(receipt, sender, dest, weight);
            }
    ...
    private void handleUpdate() {
        ...
        if (selected != null && isInputValid()) {
            ...
            BasePackage updatedPackage;
            if (type.equals("Regular Service")) {
                updatedPackage = new RegularPackage(receipt, sender, dest, weight);
            } else {
                updatedPackage = new ExpressPackage(receipt, sender, dest, weight);
            }

            service.updatePackage(index, updatedPackage);
            refreshTable();
            handleClear();
        } else {
